...
Telegram 机器人是提升群组管理效率、增强互动性的强大工具。以下是针对你需求的四大核心功能介绍：

---

## 🛠 核心功能概览

### 1. 入群欢迎与欢迎词 (Welcome Message)
当新成员加入时，Bot 会自动触发预设内容，避免群组冷场。
*   **个性化称呼**：支持通过变量（如 `{first_name}`）直接呼唤用户。

### 2. 入群验证
这是过滤“僵尸号”和“广告机器人”的最强手段。
*   **交互验证**：新成员入群后需点击按钮（如“点我验证”）或回答简单算术题。
*   **权限限制**：验证通过前，用户处于禁言状态，无法发送链接或媒体。
*   **超时踢出**：若指定时间内未完成验证，Bot 会自动将其移出群组。

### 3. 关键词自动回复 (Smart Reply)
类似于 24/7 在线客服，针对高频问题进行自动化响应。
*   **规则匹配**：支持“完全匹配”或“模糊匹配”特定关键词。
*   **多模式回复**：关键词触发后可回复文字、说明文档或跳转链接。
*   **示例**：用户发送 `官网`，Bot 立即自动回复官方网址。

### 4. 消息过滤与管理 (Message Management)
深度净化群组聊天环境，维持讨论秩序。
*   **删除转发信息**：自动检测并拦截来自其他频道（Channel）或群组的转发内容。
